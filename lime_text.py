...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
        ...
        data, yss, distances = self.__data_labels_distances(
            ...
            distance_metric=distance_metric, indexes = indexes, sep_id = sep_id)
        ...
        #return ret_exp
        ret_exps = []
        l = 1 if [] not in indexes else len(indexes)
        ...
        for i in range(l):
            ...
            if top_labels:
                ...
                ret_exp.top_labels = list(labels)
                ret_exp.top_labels.reverse()
            ret_exps.append(ret_exp)

        for label in labels:
            outs = self.base.explain_instance_with_data(
                    data, yss, distances, label, num_features,
                    model_regressor=model_regressor,
                    feature_selection=self.feature_selection,
                    indexes=indexes,
                    )
            assert len(outs) == len(ret_exps)
            for out,ret_exp in zip(outs,ret_exps):
                (ret_exp.intercept[label],
                 ret_exp.local_exp[label],
                 ret_exp.score, ret_exp.local_pred) = out
        return ret_exps
    ...
    def __data_labels_distances(self,
        ...
        doc_size2 = len(indexes)
        sample = self.random_state.randint(1, doc_size2 + 1, num_samples - 1)

        data = np.ones((num_samples, doc_size))
        data[0] = np.ones(doc_size)

        features_range = range(doc_size2)
        ...
        for i, size in enumerate(sample, start=1):
            ...
            for inac in inactive:
                inactive2.extend(indexes[inac])

            if sep_id is not None:
                while sep_id in inactive2:
                    inactive2.remove(sep_id)

            data[i, inactive2] = 0
            inverse_data.append(indexed_string.inverse_removing(inactive2))


        labels = classifier_fn(inverse_data, indexes)
