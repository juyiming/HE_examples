...
def Cal_score(id, pre_label, _example_ori, _words_ori, model, tokenizer):
    if len(id) <= 50:
        id1 = [x[0] for x in id]
        id2 = []
        for item in id:
            ...
            z.extend(y)
            id2.append(z)
        pres1 = Cal_pre(id1, pre_label, _example_ori, _words_ori, model, tokenizer)
        pres2 = Cal_pre(id2, pre_label, _example_ori, _words_ori, model, tokenizer)
        ...
        for i in range(iter_num):
            ...
            for item in id[s:e]:
                ...
                z.extend(y)
                id2.append(z)
            pres1 = Cal_pre(id1, pre_label, _example_ori, words_ori, model, tokenizer)
            pres2 = Cal_pre(id2, pre_label, _example_ori, words_ori, model, tokenizer)
            scores = [x - y for x, y in zip(pres1, pres2)]
...
if __name__ == '__main__':
    ...
    for example_ori in tqdm(dev_data[:]):
        ...
        while len(indexes) > 0:
            ...
            scores = Cal_score(cal_score_list, pre_label, example_ori, words_ori, model, tokenizer)
        ...
        out = [indexes_list, scores, words_ori]
        output.append(out)
        print('finished')
